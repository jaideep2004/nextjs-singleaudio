...
export async function GET(_request: Request, { params }: { params: Promise<{ countryIso: string }> }) {
  ...
  const response = await fetch(`${CSC_BASE_URL}/countries/${encodeURIComponent(countryIso)}/states`, {
